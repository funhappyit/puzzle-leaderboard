...
| 마이그레이션 | Flyway |
| 부하 테스트 | JMeter |
| 인프라 | Koyeb (Spring Boot), Aiven (MySQL), Upstash (Redis) |
...
[React Dashboard : Vercel]
  └── GET /api/v1/rankings  ─── 5초 폴링 ──▶ 실시간 랭킹 표시
         │ API 호출
         ▼
[Koyeb - Spring Boot : 8080]  ←── GitHub push 시 자동 배포
...
- [ ] **배포** — Koyeb + Aiven(MySQL) + Upstash(Redis) 무료 클라우드 실배포 ([#9](https://github.com/funhappyit/puzzle-leaderboard/issues/9))
